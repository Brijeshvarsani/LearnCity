...
public class DD1Eng : MonoBehaviour
{
    public int buildingNumber;
    public GameObject ratio, car, grapes, dog, sheep,photo, ratioBlack, carBlack, blockPanel, failPanel;
    ...
    public void DropGrapes()
    {
        ...
        float distance = Vector3.Distance(grapes.transform.position, ratioBlack.transform.position);
        if (distance < 50)
        {
            grapes.transform.position = ratioBlack.transform.position;
            if (ratioBlack.GetComponent<Image>().color != new Color32(96, 222, 41, 255))
            {
                Score.scoreNumber += 1;
            }
    ...
    public void DropDog()
    {
        ...
        float distance = Vector3.Distance(dog.transform.position, ratioBlack.transform.position);
        if (distance < 50)
        {
            dog.transform.position = ratioBlack.transform.position;
            dog.transform.localScale = ratioBlack.transform.localScale;
            if (ratioBlack.GetComponent<Image>().color != new Color32(96, 222, 41, 255))
            {
                Score.scoreNumber += 1;
            }
    ...
    public void DropSheep()
    {
        ...
        float distance = Vector3.Distance(sheep.transform.position, ratioBlack.transform.position);
        if (distance < 50)
        {
            sheep.transform.position = ratioBlack.transform.position;
            if (ratioBlack.GetComponent<Image>().color != new Color32(96, 222, 41, 255))
            {
                Score.scoreNumber += 1;
            }
